...
#show: aiaa.with(
    title: "Vision-Based Flow-Front Assessment in Vacuum-Assisted Resin Transfer Molding: An AIAA Template Demonstration",
    ...
    abstract: [This paper is a placeholder demonstration of the AIAA Typst template. It outlines a hypothetical computer-vision pipeline for tracking the resin flow front during Vacuum-Assisted Resin Transfer Molding (VARTM) and reports illustrative metrics on simulated data. The pipeline ingests video of a transparent vacuum bag, applies adaptive thresholding against a peak-brightness reference, and exports binary masks suitable for downstream learned segmentation. Reported numbers in this template are fabricated for formatting purposes; the example serves only to exercise the title block, abstract, nomenclature, equation, figure, and bibliography facilities. Define all symbols used in the abstract. Do not cite references in the abstract. The footnote on the first page lists the job title and AIAA member grade for each author.]
)

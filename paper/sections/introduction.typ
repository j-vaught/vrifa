...
infusion before resin gels @HickeyBickerton2012CureKinetics.
...
Recent vision-based work has begun to replace these classical operators with learned detectors @Esposito2025AIFusion @Stieber2020FlowFrontNet @Stieber2023SimToReal @Park2025FlowFrontGAN @FlowCastNet2026 and a parallel thread applies deep models to adjacent VARTM observables such as cure temperature distribution @Heuer2023VARIMTemperature. However, ML in this domain stalls on data scarcity since each VARTM run is its own regime of fabric, bag, lighting, and geometry @Malashin2025MLCompositeReview. Complicating matters further, no shared labeled video benchmark spans multiple molds, so a detector trained on one bench cannot be measured on the next @Stieber2023SimToReal. The classical-CV pipelines deploy without per-bench training data, but they too have not been evaluated against any shared video benchmark, leaving open how they compare to each other or to any future method.

...
Reference selection is computed but does not influence the delta when both peak-reference and darken-only are enabled, as in the integrated configuration. The peak map replaces $G_t$ in the darken-only formula of Section~3.7, so the menu choice above becomes load-bearing only when peak-reference or darken-only is disabled (the corresponding rows in Table~@tab:ablation).

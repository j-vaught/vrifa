...
Some anomalies deserve direct attention. First, the camera-shift registration row reports the effect of enabling registration on the integrated configuration, which does not use it by default. The drop of $0.630$ to $0.291$ is large and consistent across every sample because the per-sample dataset is mechanically stable (first-to-last pixel displacement below $4$ px on the noisiest sample and below $1$ px on every 524p clip per the dataset's image-side characterization), and on a stationary camera the registration's phase-correlation stage misfires onto small intra-laminate features (resin wetting boundaries, fabric weave alignment) that look like apparent motion. We therefore recommend that a research bench that uses a tripod-mounted camera should leave camera-shift registration off, and only enable it when there is documented camera motion exceeding the per-frame and cumulative thresholds in the configuration. Second, the peak-brightness reference is neutral on this 11-sample subset (mean $Delta$IoU $+0.009$): the per-sample numbers cancel out, with `input_1` improving by $+0.15$ and `input_6` and `input_8` each losing $-0.02$. The primitive's intended workload (lighting drift) is not the binding constraint on per-sample performance here. Third, the ROI restriction matters only for `input_1` (the only sample that uses a polygonal mask file at $-0.14$); on the ten samples that use full-frame ROI the swap is a no-op. 

== Configuration lookup

The per-sample ablation in the previous subsection is the empirical content of the paper, but the same evidence is more useful as a regime-indexed lookup that recommends preprocessing settings as a function of run circumstances. ~@tab:lookup is that lookup. Each row pairs a circumstance (illumination drift, fill rate, frame rate, camera stability) with the recommended setting on the corresponding mode menu of the method, and points at the row of~@tab:ablation or the per-sample breakdown of~@tab:agreement_per_sample that supports it. Rows whose recommendation is supported only by the eleven evaluated regimes are marked tentative (`†`); a deployed system on a circumstance the labeling subset does not cover should treat the row as a starting point for its own per-mold tuning rather than as a guarantee.
...
#figure(
  table(
    ...
    [Specular vacuum bag in field of view],                [Keep darken-only enabled],
    ...
    [Race-tracking dominates early fill],                  [First-frame reference; avoid dynamic calibration],
    [Heavily textured vacuum bag],                         [Percentile or adaptive threshold†],

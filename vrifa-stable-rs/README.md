...
- `crates/vrifa-stable-core` contains the pure algorithm stages and shared pipeline types.
- `crates/vrifa-stable-io` contains video readers, MP4 writers, and PNG writers.
- `crates/vrifa-stable-annotations` contains COCO, YOLOv5, and Darknet exporters.
- `crates/vrifa-stable-cli` contains the binary entrypoint, configuration parsing, and end-to-end orchestration.
- `crates/vrifa-stable-py` contains the optional binding crate layered over the Rust pipeline.
...
`cargo bench -p vrifa-stable-cli --bench perf_gate` runs a three-tier gate for each bundled sample video:

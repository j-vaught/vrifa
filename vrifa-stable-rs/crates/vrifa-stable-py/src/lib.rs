...
use vrifa_stable_core::{detect_front as detect_front_core, DetectFrontParams};
...
impl PyConfig {
    fn to_config(&self) -> PyResult<vrifa_stable_cli::Config> {
        let colorspace = ColorSpace::parse(&self.colorspace)
            .map_err(|err| PyValueError::new_err(err.to_string()))?;
        let mut config = vrifa_stable_cli::Config {
            ...
            ..vrifa_stable_cli::Config::default()
        };
...
fn run(config: &PyConfig) -> PyResult<()> {
    vrifa_stable_cli::run_binding_config(config.to_config()?)
...
#[pymodule]
fn vrifa_stable_py(py: Python<'_>, module: &Bound<'_, PyModule>) -> PyResult<()> {
